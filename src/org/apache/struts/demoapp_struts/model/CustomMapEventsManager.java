	...
	java.sql.ResultSet result;
		...
		public Iterable<DHXEv> getEvents() {
	        ...
	        try
	        {
	        	 ...
	        	 String query ="select event_id,start_date,end_date,text,event_location,lat,lng from events_map where userid=?";
	        	...
	        	while(res.next())
	        	{
	        		...
	        		e.setText(res.getString("event_location"));
		 ...
		 conn = database.getConnection();
			...
			if (status == DHXStatus.UPDATE)
				...
				else if (status == DHXStatus.INSERT)
				{
					...
					try
					{
					 query = "INSERT INTO events_map (event_id, text, start_date, end_date,"+
							   " event_location,lat,lng,userid) VALUES (null, ?, ?, ?, ?, ?, ?,?)";
							   				ps = conn.prepareStatement(query, Statement.RETURN_GENERATED_KEYS);
							   				ps.setString(1, e.getText());
							   				ps.setString(2, start_date);
							   				ps.setString(3, end_date);
							   				ps.setString(4, e.event_location);
							   				ps.setString(5, e.lat);
							   				ps.setString(6, e.lng);
